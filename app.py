...
st.title("💧 Monitoring Ketersediaan Air Minum Layak di Jawa Barat")
st.markdown("""
        ...
        try:
            ...
            if sumber_cols:
                ...
                if {"latitude", "longitude"}.issubset(df_filtered.columns):
                    ...
                    m = folium.Map(location=[df_filtered["latitude"].mean(), df_filtered["longitude"].mean()], zoom_start=9)
                    for _, row in df_filtered.iterrows():
                        folium.CircleMarker(
                            ...
                            fill=True,
                            fill_opacity=0.6,
                            popup=row.get("bps_nama_desa_kelurahan", "Desa/Kelurahan"),
                        ).add_to(m)
